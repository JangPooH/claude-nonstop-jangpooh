...
export function formatExtraCredit(extraUsage) {
  ...
  if (!extraUsage.is_enabled) return 'Extra credit: disabled';

  // Convert cents to dollars
  const usedDollars = (extraUsage.used_credits / 100).toFixed(2);
  const limitDollars = (extraUsage.monthly_limit / 100).toFixed(2);
  const remainingDollars = ((extraUsage.monthly_limit - extraUsage.used_credits) / 100).toFixed(2);
  // utilization is already a percentage value (0-100 range)
  const utilPercent = Number(extraUsage.utilization).toFixed(1);

  // Calculate next month's 1st day (monthly reset)
  const now = new Date();
  const nextMonth = new Date(now.getFullYear(), now.getMonth() + 1, 1, 0, 0, 0);
  const resetTime = formatResetTime(nextMonth.toISOString());

  return `Extra credit: enabled ($${usedDollars}/$${limitDollars} used, $${remainingDollars} remaining, ${utilPercent}%) / Monthly resets: ${resetTime}`;
}
