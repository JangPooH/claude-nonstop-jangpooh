...
import { createFakeGitDir, finalizeFakeGitDir, cleanupFakeGitDir, sweepStaleFakeGitDirs } from './fake-git.js';
 ...
 *   "You've hit your session limit · resets 9:10pm (Asia/Seoul)"
 */
const RATE_LIMIT_PATTERN = /(?:Limit reached|You've hit your(?: \w+)? limit)\s*[·•]\s*resets\s+(.+?)(?:\s*$|\n)/im;
...
function runOnce(claudeArgs, account, existingSessionId, options = {}) {
  return new Promise((resolve) => {
    const fakeGit = createFakeGitDir(process.cwd());

    const env = {
      ...process.env,
      CLAUDE_CONFIG_DIR: account.configDir,
      FORCE_COLOR: '1',
      PATH: `${fakeGit.dir}:${process.env.PATH}`,
      CLAUDE_NONSTOP_FAKE_GIT_DIR: fakeGit.dir,
      CLAUDE_NONSTOP_REAL_GIT: fakeGit.realGitPath,
    };
    ...
    finalizeFakeGitDir(fakeGit.dir, child.pid);
