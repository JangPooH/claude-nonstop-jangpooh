...
export function migrateSessionByHash(fromConfigDir, toConfigDir, cwdHash, sessionId) {
  ...
  try {
    ...
    // profiles/*/projects is normally a symlink into the same shared
    // projects dir (see setup), so fromProjectDir and toProjectDir resolve
    // to the same real path and no copy is needed — this is the common case.
    // Only distinct real dirs (e.g. an unlinked/custom profile) get copied.
